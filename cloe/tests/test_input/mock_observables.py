...
def build_mock_observables(reader=None):
    ...
    for redshift_index, redshift in enumerate(redshifts):
        multipoles = [
            key
            for key in reader.data_dict["GC-Spectro"][f"{redshift}"].keys()
        ...
        gc_spectro_configuration_space_bins[f"n{redshift_index+1}"] = {
            f"n{redshift_index+1}": {"multipoles": multipole_bins_configuration_space}
        }
    gc_spectro_specs = {
        "statistics": "multipole_power_spectrum",
        "multipole_correlation_function": {"bins": gc_spectro_configuration_space_bins},
    }
