...
class yaml_handler_test(TestCase):
    ...
    def test_write_bad_input_type(self):
        self.assertRaises(
            TypeError, yaml_write, self.file_name, self.bad_input_type, self.overwrite
        )

    def test_write_overwrite_exception(self):
        self.assertRaises(RuntimeError, yaml_write, self.file_name, self.config)
    ...
    def test_read_and_check_dict_bad_type(self, load_mock):
        ...
        self.assertRaises(TypeError, yaml_read_and_check_dict, self.file_name, None)

    @patch("yaml.load")
    def test_read_and_check_dict_bad_dict(self, load_mock):
        load_mock.return_value = self.config
        self.assertRaises(
            KeyError, yaml_read_and_check_dict, self.file_name, ["missing key"]
        )
