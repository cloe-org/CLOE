...
class cosmoinitTestCase(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.cosmo.nonlinear.theory["redshift_bins_means_spectro"] = cls.cosmo.cosmo_dic[
        ...
        cls.cosmo_curv_neg.cosmo_dic = load_test_pickle(
            "cosmo_test_curv_neg_dic.pickle"
        )
        # Define test case for positive curvature
        cls.cosmo_curv_pos = Cosmology()
        cls.cosmo_curv_pos.cosmo_dic = load_test_pickle(
            "cosmo_test_curv_pos_dic.pickle"
        )
        # Define test case for gamma parametrization
        cls.cosmo_gamma = Cosmology()
        cls.cosmo_gamma.cosmo_dic = load_test_pickle("cosmo_test_gamma_dic.pickle")
        # Define test case for nonlinear model
        cls.cosmo_NL = Cosmology()
        cls.cosmo_NL.cosmo_dic = load_test_pickle("cosmo_test_NLphot2_gamma_dic.pickle")
    ...
    def test_cosmo_nuisance_init(self):
        ...
        npt.assert_equal(
            ...
            True,
            err_msg=(
                "nuisance parameters not initialized within cosmology " + "dictionary"
            ),
        )
    ...
    def test_transverse_comoving_dist_z12(self):
        ...
        npt.assert_allclose(
            ...
            rtol=1e-3,
            err_msg=(
                "Error in transverse comoving distance (z1/z2) " + "interpolation (K=0)"
            ),
        )
    ...
    def test_compute_phot_galbias(self):
        ...
        self.cosmo.create_phot_galbias(
            model=2, x_values=zs_means, y_values=[2.0, 4.0, 6.0]
        )
        ...
        bi_val_actual = self.cosmo.compute_phot_galbias(0.98)
        npt.assert_almost_equal(
            bi_val_actual,
            desired=2.0,
            decimal=3,
    ...
    def test_MG_sigma_def(self):
        test_sigma = self.cosmo.MG_sigma_def(1.0, 0.01, self.MG_sigma_test)
        npt.assert_equal(
            test_sigma,
            self.MG_sigma_test,
    ...
    def test_matter_density(self):
        test_matter_density = self.cosmo.matter_density(self.cosmo.cosmo_dic["z_win"])
        npt.assert_allclose(
