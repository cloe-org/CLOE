...
class ConfigGUIWindow(QtWidgets.QMainWindow):
    ...
    def read_cosmo_pars(self, latex_name, max, min, proposal, fiducial, scale):
        if max != "" and min != "" and proposal != "":
            read_values = {
                "latex": latex_name,
                "prior": {"max": float(max), "min": float(min)},
                "proposal": float(proposal),
                "ref": {"dist": "norm", "loc": float(fiducial)},
                "scale": float(scale),
            }
        else:
    ...
    def dumpToYAMLAndExit(self):
        dictionary_WL = {
            ...
            "bins": {
                ...
                + str(i + 1): {
                    ...
                    + str(j + 1): {
                        "ell_range": [
                            [int(self.ui.Min_l_WL.text()), int(self.ui.Max_l_WL.text())]
                        ]
                    }
                    for j in range(i, 10)
                }
            ...
            "bins": {
                ...
                + str(i + 1): {
                    ...
                    + str(j + 1): {
                        "ell_range": [
                            [
                                int(self.ui.Min_l_GCph.text()),
                                int(self.ui.Max_l_GCph.text()),
                            ]
                        ]
                    }
                    for j in range(i, 10)
                    ...
                    "n1": {
                        "multipoles": {
                            0: {
                                "k_range": [
                                    [
                                        float(self.ui.GCsp_k_min.text()),
                                        float(self.ui.GCsp_k_max.text()),
                                    ]
                                ]
                            },
                            2: {
        ...
        with open("../configs/WL-FourierSpace.yaml", "w") as yaml_file:
            yaml.dump(
                dictionary_WL, yaml_file, default_flow_style=None, sort_keys=False
            )
        with open("../configs/GCphot-FourierSpace.yaml", "w") as yaml_file:
            yaml.dump(
                dictionary_GCph, yaml_file, default_flow_style=None, sort_keys=False
            )
        with open("../configs/GCspectro-FourierSpace.yaml", "w") as yaml_file:
            yaml.dump(
                dictionary_GCsp, yaml_file, default_flow_style=None, sort_keys=False
            )
        ...
        with open("../configs/models/cosmology.yaml", "w") as yaml_file:
            yaml.dump(
                dictionary_cosmo, yaml_file, default_flow_style=None, sort_keys=False
            )
        with open("../configs/data.yaml", "w") as yaml_file:
            yaml.dump(
                dictionary_data, yaml_file, default_flow_style=None, sort_keys=False
            )
