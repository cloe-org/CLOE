...
class datareaderTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.cov_check_GC_spectro = 1.217193e08
    ...
    def test_nz_WL_dict_init(self):
        ...
        npt.assert_equal(
            true_nz_dict,
            self.nz_key_check,
            err_msg="Error in data reading nz WL " "dict initialisation.",
        )
    ...
    def test_nz_GC_Phot_dict_init(self):
        ...
        npt.assert_equal(
            true_nz_dict,
            self.nz_key_check,
            err_msg="Error in data reading nz GC Phot " "dict initialisation.",
        )

    def test_nz_GC_Phot_dict_interpolator(self):
        npt.assert_allclose(
            self.data_tester.nz_dict_GC_Phot["n1"](0.399987),
            self.nz_dict_GC_Phot_check,
            atol=1e-06,
            err_msg="Error in the interpolation of " "raw n(z) GC data",
        )

    def test_nz_WL_dict_interpolator(self):
        npt.assert_allclose(
            self.data_tester.nz_dict_WL["n1"](0.399987),
            self.nz_dict_WL_check,
            atol=1e-06,
            err_msg="Error in the interpolation of " "raw n(z) WL data",
        )
    ...
    def test_bench_gc_cov_check(self):
        ...
        npt.assert_allclose(
            test_cov,
            self.cov_check_GC_spectro,
            rtol=1e-3,
            err_msg="Error in loading external spectroscopic" " test data.",
        )
    ...
    def test_bench_phot_cov_check(self):
        ...
        test_cov_3x2pt = np.sum(
            np.outer(np.arange(shape_3x2pt), np.arange(shape_3x2pt))
            * self.data_tester.data_dict["3x2pt_cov"]
        )
        npt.assert_allclose(
            test_cov_3x2pt,
            [self.cov_check_3x2pt],
            rtol=1e-3,
            err_msg="Error in loading external photometric" " test covariance data.",
        )
    ...
    def test_bench_phot_cls_check(self):
        ...
        npt.assert_allclose(
            [test_Cl_WL, test_Cl_GC, test_Cl_XC],
            [
                self.cl_phot_WL_check_std,
                self.cl_phot_GC_check_std,
                self.cl_phot_XC_check_std,
            ],
            rtol=1e-3,
    ...
    def test_fiducial_reading_check(self):
        ...
        self.data_tester.read_GC_spectro()
        npt.assert_equal(
            list(self.data_tester.data_spectro_fiducial_cosmo.keys()),
            self.fiducial_key_check,
        )
    ...
    def test_scale_cut_reading_check(self):
        ...
        npt.assert_equal(
            list(self.data_tester.GC_spectro_scale_cuts.keys()),
