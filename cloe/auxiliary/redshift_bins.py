...
def linear_interpolator(x_values, y_values):
    ...
    return interpolate.interp1d(
        x_values, y_values, fill_value=(y_values[0], y_values[-1]), bounds_error=False
    )
...
def select_spectro_parameters(redshift, nuis_dict, bin_edges=None):
    ...
    try:
        ...
        b1 = np.array([nuis_dict[f"b1_spectro_bin{i}"] for i in np.nditer(z_bin)])
        b2 = np.array([nuis_dict[f"b2_spectro_bin{i}"] for i in np.nditer(z_bin)])
        bG2 = np.array([nuis_dict[f"bG2_spectro_bin{i}"] for i in np.nditer(z_bin)])
        bG3 = np.array([nuis_dict[f"bG3_spectro_bin{i}"] for i in np.nditer(z_bin)])
        c0 = np.array([nuis_dict[f"c0_spectro_bin{i}"] for i in np.nditer(z_bin)])
        c2 = np.array([nuis_dict[f"c2_spectro_bin{i}"] for i in np.nditer(z_bin)])
        c4 = np.array([nuis_dict[f"c4_spectro_bin{i}"] for i in np.nditer(z_bin)])
        ck4 = np.array([nuis_dict[f"ck4_spectro_bin{i}"] for i in np.nditer(z_bin)])
        aP = np.array([nuis_dict[f"aP_spectro_bin{i}"] for i in np.nditer(z_bin)])
        e0k2 = np.array([nuis_dict[f"e0k2_spectro_bin{i}"] for i in np.nditer(z_bin)])
        e2k2 = np.array([nuis_dict[f"e2k2_spectro_bin{i}"] for i in np.nditer(z_bin)])
        Psn = np.array([nuis_dict[f"Psn_spectro_bin{i}"] for i in np.nditer(z_bin)])
        ...
        raise KeyError(
            "Input nuisance parameter dictionary does not "
