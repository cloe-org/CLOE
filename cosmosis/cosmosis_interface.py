...
def setup(options):
    ...
    config_file = options.get_string(option_section, "config_file")
    ...
    observables = observables_selection_specifications_checker(
        info["observables_selection"], info["observables_specifications"]
    )
    if info["plot_observables_selection"]:
        observables_pf = observables_visualization(observables["selection"])
    ...
    cloe_cosmo.cosmo_dic["k_win"] = np.logspace(
        np.log10(info["k_min_extrap"]), np.log10(info["k_max_extrap"]), info["k_samp"]
    )
    ...
    if info["observables_selection"]["CMBlens"]["CMBlens"]:
        ...
        cloe_cosmo.cosmo_dic["z_win_max"] = np.logspace(
            ...
            np.log10(cloe_cosmo.cosmo_dic["z_max_cmb"]),
            cloe_cosmo.cosmo_dic["z_samp_log"],
        )
        cloe_cosmo.cosmo_dic["z_win_max"] = np.unique(
            np.append(cloe_cosmo.cosmo_dic["z_win"], cloe_cosmo.cosmo_dic["z_win_max"])
        )

    # read in options from yaml dict, store in cosmo dict
    cloe_cosmo.cosmo_dic["NL_flag_phot_matter"] = info["NL_flag_phot_matter"]
    ...
    cloe_cosmo.cosmo_dic["NL_flag_phot_baryon"] = info["NL_flag_phot_baryon"]
    cloe_cosmo.cosmo_dic["Baryon_redshift_model"] = info["Baryon_redshift_model"]
    ...
    cloe_cosmo.cosmo_dic["matrix_transform_phot"] = info["observables_selection"][
    ...
    cloe_cosmo.cosmo_dic["redshift_bins_means_phot"] = data["photo"]["redshifts"]
    cloe_cosmo.nonlinear = Nonlinear(cloe_cosmo.cosmo_dic)
    cloe_cosmo.nonlinear.theory["redshift_bins_means_spectro"] = data["spectro"][
        "edges"
    ]
    cloe_cosmo.nonlinear.set_Pgg_spectro_model()
    cloe_cosmo.cosmo_dic["redshift_bins_means_spectro"] = data["spectro"]["edges"]
    ...
    likefinal.fiducial_cosmo_quantities_dic.update(fid_cosmo.cosmo_dic)
    ...
    if not any(info["observables_selection"]["CG"].values()):
        ...
        if (
            any(info["observables_selection"]["WL"].values())
            or any(info["observables_selection"]["GCphot"].values())
            or any(info["observables_selection"]["GCspectro"].values())
        ):
            raise ValueError("Galaxy cluster probes cannot be combined with others.")
    ...
    cloe_cosmo.cosmo_dic["r_win"] = np.linspace(5, 50, 100)
...
def set_fiducial_cosmology(likefinal, info):
    ...
    if info["observables_selection"]["CMBlens"]["CMBlens"]:
        fid_cosmo.cosmo_dic["z_win_max"] = np.logspace(
            ...
            np.log10(fid_cosmo.cosmo_dic["z_max_cmb"]),
            fid_cosmo.cosmo_dic["z_samp_log"],
        )
        fid_cosmo.cosmo_dic["z_win_max"] = np.unique(
            np.append(fid_cosmo.cosmo_dic["z_win"], fid_cosmo.cosmo_dic["z_win_max"])
        )
    fid_cosmo.cosmo_dic["k_win"] = np.logspace(
        np.log10(info["k_min_extrap"]), np.log10(info["k_max_extrap"]), info["k_samp"]
    )
    ...
    fid_cosmo.cosmo_dic["r_win"] = np.linspace(5, 50, 100)
    fid_cosmo.cosmo_dic.update(likefinal.data_spectro_fiducial_cosmo)
    ...
    pars.InitPower.set_params(
        As=fid_cosmo.cosmo_dic["As"], ns=fid_cosmo.cosmo_dic["ns"], r=0
    )
    pars.set_matter_power(
        redshifts=fid_cosmo.cosmo_dic["z_win_max"],
    ...
    # Update fiducial cosmology dictionary
    fid_cosmo.cosmo_dic["Omm"] = pars.omegam
    fid_cosmo.cosmo_dic["Omk"] = pars.omk
    fid_cosmo.cosmo_dic["comov_dist"] = results.comoving_radial_distance(
        fid_cosmo.cosmo_dic["z_win_max"]
    )
    fid_cosmo.cosmo_dic["angular_dist"] = results.angular_diameter_distance(
        fid_cosmo.cosmo_dic["z_win"]
    )
    fid_cosmo.cosmo_dic["H"] = results.hubble_parameter(fid_cosmo.cosmo_dic["z_win"])
    fid_cosmo.cosmo_dic["H_Mpc"] = results.h_of_z(fid_cosmo.cosmo_dic["z_win"])
    fid_cosmo.cosmo_dic["Pk_delta_Boltzmann"] = results.get_matter_power_interpolator(
        nonlinear=False,
        var1="delta_tot",
        var2="delta_tot",
        extrap_kmax=fid_cosmo.cosmo_dic["k_win"][-1],
    )
    fid_cosmo.cosmo_dic["Pk_cb_Boltzmann"] = results.get_matter_power_interpolator(
        nonlinear=False,
        var1="delta_nonu",
        var2="delta_nonu",
        extrap_kmax=fid_cosmo.cosmo_dic["k_win"][-1],
    )
    fid_cosmo.cosmo_dic["Pk_weyl"] = results.get_matter_power_interpolator(
        nonlinear=False,
        var1="Weyl",
        var2="Weyl",
        extrap_kmax=fid_cosmo.cosmo_dic["k_win"][-1],
    ...
    fid_cosmo.cosmo_dic["fsigma8"] = results.get_fsigma8()[::-1]
    fid_cosmo.cosmo_dic["sigma8"] = results.get_sigma8()[::-1]
    # For galaxy clusters
    fid_cosmo.cosmo_dic["R"] = fid_cosmo.cosmo_dic["r_win"]
    fid_cosmo.cosmo_dic["sigmaR"] = results.get_sigmaR(
        fid_cosmo.cosmo_dic["r_win"], var1="delta_tot", var2="delta_tot"
    )[::-1]
    fid_cosmo.cosmo_dic["sigmaR_cb"] = results.get_sigmaR(
        fid_cosmo.cosmo_dic["r_win"], var1="delta_nonu", var2="delta_nonu"
    )[::-1]
    ...
    fid_cosmo.cosmo_dic["redshift_bins_means_phot"] = info["data"]["photo"]["redshifts"]
    fid_cosmo.nonlinear.theory["redshift_bins_means_spectro"] = info["data"]["spectro"][
        "edges"
    ]
    ...
    fid_cosmo.update_cosmo_dic(fid_cosmo.cosmo_dic["z_win"], 0.05)
...
def execute(block, config):
    ...
    try:
        ...
        cloe_cosmo.cosmo_dic["Pk_delta_Boltzmann"] = PowerSpectrumInterpolator(
            z, k * block[cosmo, "h0"], pk_delta / h3
        )
        z, k, pk_cb = block.get_grid("cdm_baryon_power_lin", "z", "k_h", "p_k")
        cloe_cosmo.cosmo_dic["Pk_cb_Boltzmann"] = PowerSpectrumInterpolator(
            z, k * block[cosmo, "h0"], pk_cb / h3
        )
        z, k, pk_weyl = block.get_grid("weyl_curvature_power_lin", "z", "k_h", "p_k")
        cloe_cosmo.cosmo_dic["Pk_weyl"] = PowerSpectrumInterpolator(
            z, k * block[cosmo, "h0"], pk_weyl / h3
        )
        if cloe_cosmo.cosmo_dic["NL_flag_phot_matter"] > 0:
            z, k, pk_delta_nl = block.get_grid("matter_power_nl", "z", "k_h", "p_k")
            cloe_cosmo.cosmo_dic["Pk_halomodel_recipe_Boltzmann"] = (
                PowerSpectrumInterpolator(z, k * block[cosmo, "h0"], pk_delta_nl / h3)
            )
            z, k, pk_weyl_nl = block.get_grid(
                "weyl_curvature_power_nl", "z", "k_h", "p_k"
            )
            cloe_cosmo.cosmo_dic["Pk_weyl_NL"] = PowerSpectrumInterpolator(
                z, k * block[cosmo, "h0"], pk_weyl_nl / h3
            )
    ...
    for nuisance_param in nuisance_params:
        ...
        if "hmcode_logt_agn" in nuisance_param:
            nuisance_param = nuisance_param.replace(
