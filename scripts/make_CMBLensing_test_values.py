...
zs_base = np.unique(
    np.append(
        np.linspace(0.0, 4.0, 100),
        np.logspace(np.log10(4.0), np.log10(z_max), z_samp_log),
    )
)
...
np.savetxt(
    str(cur_dir) + "/../likelihood/tests/test_input_cmbx/ComDist-LCDM-Lin-zNLA.dat",
    np.column_stack((zs_base, cmov)),
)

Hz = model.provider.get_Hubble(zs_base)
np.savetxt(
    str(cur_dir) + "/../likelihood/tests/test_input_cmbx/Hz.dat",
    np.column_stack((zs_base, Hz)),
)
...
win_kCMBcheck = cmblens.kCMB_window(2.0)
