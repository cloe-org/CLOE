...
class Euclike:
    ...
    def __init__(self, data, observables):
        ...
        self.do_photo = any(observables["selection"]["WL"].values()) or any(
            observables["selection"]["GCphot"].values()
        )
        try:
            self.do_cmbx = any(observables["selection"]["CMBlens"].values()) or any(
                observables["selection"]["CMBisw"].values()
            )
        except KeyError:
            self.do_cmbx = False
        ...
        if self.do_spectro:
            if (
                ...
                raise ValueError(
                    "Unknown statistics_spectro choice. "
        ...
        if self.do_photo:
            ...
            if (
                ...
                raise ValueError(
                    "Unknown statistics_photo choice. "
        ...
        self.data_spectro_fiducial_cosmo = self.data_ins.data_spectro_fiducial_cosmo
        ...
        if self.do_photo:
            ...
            x_full_xc = np.indices((numtomo_gcphot, numtomo_wl)).reshape(2, -1) + 1
            self.indices_diagonal_wl = tuple(zip(x_diagonal_wl[0], x_diagonal_wl[1]))
            self.indices_diagonal_gcphot = tuple(
                zip(x_diagonal_gcphot[0], x_diagonal_gcphot[1])
            )
            self.indices_all = tuple(zip(x_full_xc[0], x_full_xc[1]))
            self.scales_WL = self.data_ins.data_dict["WL"][self.scale_var_photo]
            self.scales_XC = self.data_ins.data_dict["XC-Phot"][self.scale_var_photo]
            self.scales_GC_phot = self.data_ins.data_dict["GC-Phot"][
                self.scale_var_photo
            ]
            ...
            # interface
            self.matrix_transform_phot = self.observables["selection"][
            ...
            theta_arcmin = np.logspace(
                np.log10(theta_min_arcmin), np.log10(theta_max_arcmin), nbins_theta
            )
            ...
            if self.do_fourier_photo:
                ...
                self.phot_ins.set_prefactor(
                    ells_WL=self.scales_WL,
        ...
        if self.do_spectro:
            ...
            self.data_spectro_fiducial_cosmo = self.data_ins.data_spectro_fiducial_cosmo
            self.mixing_matrix_dict = self.data_ins.read_GC_spectro_mixing_matrix()
            self.zkeys = self.data_ins.data_dict["GC-Spectro"].keys()
            # Spectro class instance
            self.spec_ins = Spectro(None, list(self.zkeys), self.mixing_matrix_dict)
    ...
    def get_masked_data(self):
        ...
        if self.do_photo and self.do_spectro:
            datafinal = {**phot_data, "GC-Spectro": spectrodata}
            covfinal = {
                "3x2pt": self.data_ins.data_dict["3x2pt_cov"],
        ...
        self.data_handler_ins = Data_handler(
            datafinal, covfinal, self.observables, self.data_ins
        )

        # Mask data vectors and covariances
        # for the photometric and spectroscopic probes separately
        if self.do_photo:
            self.data_vector_phot, self.cov_matrix_phot, self.masking_vector_phot = (
            ...
            self.masked_data_vector_phot = self.mask_ins_phot.get_masked_data_vector()
            ...
            if self.data["photo"]["cov_is_num"]:
                ...
                if self.nsim_phot <= self.ndata_phot + 1.0:
                    ...
                    raise ValueError(
                        "Cannot apply Percival et al. 2022 "
                        "likelihood shape for photo "
                        "because cov_nsim is too low"
                    )
            self.masked_invcov_matrix_phot = np.linalg.inv(self.masked_cov_matrix_phot)
            # Check for inversion issues
            if not np.allclose(
                np.dot(self.masked_cov_matrix_phot, self.masked_invcov_matrix_phot),
                np.eye(self.masked_cov_matrix_phot.shape[0]),
                atol=1e-7,
            ):
                raise ValueError(
                    "Problem with the inversion of the " "photo covariance"
                )
        if self.do_spectro:
            (
                self.data_vector_spectro,
            ...
            self.mask_ins_spectro.set_covariance_matrix(self.cov_matrix_spectro)
            self.mask_ins_spectro.set_masking_vector(self.masking_vector_spectro)
            ...
            self.masked_invcov_matrix_spectro = np.linalg.inv(
                self.masked_cov_matrix_spectro
            )
            # Check for inversion issues
            if not np.allclose(
                np.dot(
                    self.masked_cov_matrix_spectro, self.masked_invcov_matrix_spectro
                ),
                np.eye(self.masked_cov_matrix_spectro.shape[0]),
            ):
                raise ValueError(
                    "Problem with the inversion of the " "spectro covariance"
                )
    ...
    def create_photo_data(self):
        ...
        datavec_dict["WL"] = self.transform_photo_theory_data_vector(
            datavec_dict["WL"], obs="WL"
        )
        datavec_dict["XC-Phot"] = self.transform_photo_theory_data_vector(
            datavec_dict["XC-Phot"], obs="XC-phot"
        )
        datavec_dict["GC-Phot"] = self.transform_photo_theory_data_vector(
            datavec_dict["GC-Phot"], obs="GC-phot"
        )
        datavec_dict["all"] = np.concatenate(
            (
                datavec_dict["WL"],
    ...
    def create_photo_theory(self, dictionary):
        ...
        if self.data_handler_ins.use_wl:
            if self.do_fourier_photo:
                wl_array = np.array(
                    [
                        self.phot_ins.Cl_WL(self.scales_WL, element[0], element[1])
                ...
                wl_xi_plus = np.array(
                    [
                        self.phot_ins.corr_func_3x2pt(
                            "shear-shear_plus", self.scales_WL, element[0], element[1]
                        )
                        for element in self.indices_diagonal_wl
                    ]
                ).flatten("F")
                wl_xi_minus = np.array(
                    [
                        self.phot_ins.corr_func_3x2pt(
                            "shear-shear_minus", self.scales_WL, element[0], element[1]
                        )
                        for element in self.indices_diagonal_wl
                    ]
                ).flatten("F")
            ...
            wl_array = np.zeros(
                self.num_wl_obs * len(self.scales_WL) * len(self.indices_diagonal_wl)
            )
        ...
        if self.data_handler_ins.use_xc_phot:
            if self.do_fourier_photo:
                xc_phot_array = np.array(
                    [
                        self.phot_ins.Cl_cross(self.scales_XC, element[1], element[0])
                ...
                xc_phot_array = np.array(
                    [
                        self.phot_ins.corr_func_3x2pt(
                            "shear-position", self.scales_XC, element[1], element[0]
                        )
                        for element in self.indices_all
                    ]
                ).flatten("F")
        else:
            xc_phot_array = np.zeros(len(self.scales_XC) * len(self.indices_all))
        ...
        if self.data_handler_ins.use_gc_phot:
            if self.do_fourier_photo:
                gc_phot_array = np.array(
                    [
                        self.phot_ins.Cl_GC_phot(
                            self.scales_GC_phot, element[0], element[1]
                        )
            ...
            gc_phot_array = np.zeros(
                len(self.scales_GC_phot) * len(self.indices_diagonal_gcphot)
            )

        # Apply any matrix transform activated with a switch
        wl_array = self.transform_photo_theory_data_vector(wl_array, obs="WL")
        xc_phot_array = self.transform_photo_theory_data_vector(
            xc_phot_array, obs="XC-phot"
        )
        gc_phot_array = self.transform_photo_theory_data_vector(
            gc_phot_array, obs="GC-phot"
        )
    ...
    def create_photoxcmb_data(self):
        ...
        self.tomo_ind_kCMBxWL = list(self.data_ins.data_dict["kCMBxWL"].keys())[1:]
        CMBX_dict["kCMBxWL"] = np.array(
            [
                self.data_ins.data_dict["kCMBxWL"][key][scale]
                for scale in range(len(self.data_ins.data_dict["kCMBxWL"]["ells"]))
                for key in self.tomo_ind_kCMBxWL
            ]
        )

        self.tomo_ind_kCMBxGC = list(self.data_ins.data_dict["kCMBxGC"].keys())[1:]
        CMBX_dict["kCMBxGC"] = np.array(
            [
                self.data_ins.data_dict["kCMBxGC"][key][scale]
                for scale in range(len(self.data_ins.data_dict["kCMBxGC"]["ells"]))
                for key in self.tomo_ind_kCMBxGC
            ]
        )

        self.tomo_ind_ISWxGC = list(self.data_ins.data_dict["ISWxGC"].keys())[1:]
        CMBX_dict["ISWxGC"] = np.array(
            [
                self.data_ins.data_dict["ISWxGC"][key][scale]
                for scale in range(len(self.data_ins.data_dict["ISWxGC"]["ells"]))
    ...
    def create_photoxcmb_theory(self):
        ...
        if self.data_handler_ins.use_kCMB:
            kCMB_array = self.cmbx_ins.Cl_kCMB(self.data_ins.data_dict["kCMB"]["ells"])
        else:
        ...
        cmbx_theory_vec = np.concatenate(
            (kCMB_array, kCMBxWL_array, kCMBxGC_array, iswxgc_array), axis=0
        )
    ...
    def precompute_matrix_transform_phot(self):
        ...
        if self.do_photo:
            if not self.matrix_transform_phot:
                ...
                nuisance_dict = self.fiducial_cosmo_quantities_dic[
                    "nuisance_parameters"
                ]
                nz_WL = RedshiftDistribution(
                    "WL", self.phot_ins.nz_dic_WL, nuisance_dict
                )
                chiwin = self.fiducial_cosmo_quantities_dic["r_z_func"](zwin)
                Nz = nz_WL.get_num_tomographic_bins()
                ni_list = np.array(
                    [nz_WL.interpolates_n_i(ni + 1, zwin)(zwin) for ni in range(Nz)]
                )
                if "test" in self.matrix_transform_phot:
                    ...
                    test_BNT = False
                self.BNT_transformation = BNT_transform(
                    zwin, chiwin, ni_list, test_unity=test_BNT
                )
            else:
    ...
    def transform_photo_theory_data_vector(self, obs_array, obs="WL"):
        ...
        if not self.matrix_transform_phot:
            ...
            if obs == "WL":
                ...
                transformed_array = (
                    self.BNT_transformation.apply_vectorized_symmetric_BNT(
                        N_scales, obs_array
                    )
                )
            elif obs == "XC-phot":
                ...
                transformed_array = (
                    self.BNT_transformation.apply_vectorized_nonsymmetric_BNT(
                        N_scales, obs_array
                    )
                )
            elif obs == "GC-phot":
    ...
    def create_spectro_theory(self, dictionary):
        ...
        for z_ins in self.zkeys:
            m_ins = [
                int(str(key)[-1])
                for key in self.data_ins.data_dict["GC-Spectro"][z_ins].keys()
                if key.startswith(self.str_start_spectro)
            ]
            if self.do_fourier_spectro:
                k_m_matrix = []
                for scale_ins in self.data_ins.data_dict["GC-Spectro"][z_ins][
                    self.scale_var_spectro
                ]:
                    k_m_matrix.append(
                        self.spec_ins.multipole_spectra(
                            float(z_ins), scale_ins, ms=m_ins
                        )
                    )
    ...
    def create_spectro_data(self):
        ...
        for z_ins in self.zkeys:
            multipoles = [
                k
                for k in self.data_ins.data_dict["GC-Spectro"][z_ins].keys()
                if k.startswith(self.str_start_spectro)
            ]
            for m_ins in multipoles:
                datavec = np.append(
                    datavec, self.data_ins.data_dict["GC-Spectro"][z_ins][m_ins]
                )
        return datavec
    ...
    def create_spectro_cov(self):
        ...
        for z_ins in self.zkeys:
            ...
            covnumsc.append(
                num_multipoles
                * len(
                    self.data_ins.data_dict["GC-Spectro"][z_ins][self.scale_var_spectro]
                )
            )
        ...
        for z_ins in self.zkeys:
            c1 = c1 + covnumsc[kc]
            c2 = c2 + covnumsc[kc + 1]
            covfull[c1:c2, c1:c2] = self.data_ins.data_dict["GC-Spectro"][z_ins]["cov"]
            kc = kc + 1
    ...
    def create_CG_theory(self, dictionary):
        ...
        CG_like_selection = self.observables["specifications"]["CG"]["CG_probe"]
    ...
    def create_CG_cov_analytic(self, CG_xi2_cov_selection):
                ...
                for j in range(theoryvec_cov_buf.shape[2]):
                    for k in range(theoryvec_cov_buf.shape[1]):
                        for ll in range(theoryvec_cov_buf.shape[3]):
                            ...
                            theoryvec_CC_cov[idx1][idx2] = theoryvec_cov_buf[
                                i, k, j, ll
                            ]
        ...
        if CG_xi2_cov_selection == "covCC_covCxi2":
            return np.linalg.inv(theoryvec_CC_cov), np.linalg.inv(theoryvec_xi2_cov)
    ...
    def create_CG_cov_external(self):
        ...
        for i in range(self.data_ins.data_dict["CG_xi2"].shape[0]):
            for j in range(self.data_ins.data_dict["CG_xi2"].shape[1]):
                for k in range(self.data_ins.data_dict["CG_xi2"].shape[2]):
                    n = n + 1
                    covfull_xi2[n] = self.data_ins.data_dict["CG_cov_xi2"][i][j][k]
    ...
    def loglike(self, dictionary, npar=None):
        ...
        if self.do_photo:
            ...
            chi2_phot = np.dot(
                np.dot(masked_data_minus_theory_phot, self.masked_invcov_matrix_phot),
                masked_data_minus_theory_phot,
            )
            ...
            if self.data["photo"]["cov_is_num"]:
                ...
                m_phot = (
                    npar
                    + 2.0
                    + (self.nsim_phot - 1.0 + B_phot * (self.ndata_phot - npar))
                    / (1.0 + B_phot * (self.ndata_phot - npar))
                )
                loglike_phot = (
                    -m_phot / 2.0 * np.log(1.0 + chi2_phot / (self.nsim_phot - 1.0))
                )
            else:
        ...
        if self.do_spectro:
            ...
            chi2_spectro = np.dot(
                np.dot(
                    masked_data_minus_theory_spectro, self.masked_invcov_matrix_spectro
                ),
                masked_data_minus_theory_spectro,
            )
            # If the covariance is numerical we use the non-Gaussian likelihood
            ...
            if self.data["spectro"]["cov_is_num"]:
                ...
                loglike_spectro = (
                    -m_spectro
                    / 2.0
                    * np.log(1.0 + chi2_spectro / (self.nsim_spectro - 1.0))
                )
            else:
        ...
        if self.do_clusters:
            ...
            if CG_like_selection == "CC":
                self.CGCCthvec = self.create_CG_theory(dictionary)
                dmt = self.CGCCdatafinal - self.CGCCthvec
                if CG_xi2_cov_selection == "covCC":
                    self.CGinvcovCCfinal = self.create_CG_cov_analytic(
                ...
                createCGtheory = self.create_CG_theory(dictionary)
                ...
                if CG_xi2_cov_selection == "covCC":
                    ...
                    self.xi2invcovCCfinal = self.create_CG_cov_analytic(
                        CG_xi2_cov_selection
                    )
                    loglike_clusters = -0.5 * np.sum(
                        (dmtCC * self.CGinvcovCCfinal) ** 2.0
                    ) - 0.5 * np.dot(np.dot(dmtxi2, self.xi2invcovCCfinal), dmtxi2)
                    ...
                    err_msg = "Choose CG covariance selection "
                    err_msg += "'CG_nonanalytic_cov' or" + " 'covCxi2' or 'covCC'"
                ...
                if CG_xi2_cov_selection == "covCC":
                    ...
                    loglike_clusters = (
                        -0.5 * np.dot(np.dot(dmtCC, self.CGinvcovCCfinal), dmtCC)
                        - 0.5 * np.sum((dmtMoR * self.CGinvcovMoRfinal) ** 2.0)
                        - 0.5 * np.sum((dmtxi2 * self.CGinvcovxi2final) ** 2.0)
                    )
