...
for bin_i in range(1, nbin + 1):
    for bin_j in range(1, nbin + 1):
        if bin_j >= bin_i:
            cl_GC = np.array(
                [photo.Cl_GC_phot(cur_ell, bin_i, bin_j) for cur_ell in ells]
            )
            cl_WL = np.array([photo.Cl_WL(cur_ell, bin_i, bin_j) for cur_ell in ells])
            matGC.append(cl_GC)
            matWL.append(cl_WL)
        cl_cross = np.array([photo.Cl_cross(cur_ell, bin_j, bin_i) for cur_ell in ells])
...
txtstr = "NL_flag_phot_matter_{:d}".format(
    info["likelihood"]["Euclid"]["NL_flag_phot_matter"]
...
np.savetxt(
    data_path + "Cls_zNLA_PosPos_" + txtstr + ".dat",
    matGC,
    fmt="%.12e",
    delimiter="\t",
    newline="\n",
...
np.savetxt(
    data_path + "Cls_zNLA_ShearShear_" + txtstr + ".dat",
    matWL,
    fmt="%.12e",
    delimiter="\t",
    newline="\n",
...
np.savetxt(
    data_path + "Cls_zNLA_PosShear_" + txtstr + ".dat",
    matXC,
    fmt="%.12e",
    delimiter="\t",
    newline="\n",
