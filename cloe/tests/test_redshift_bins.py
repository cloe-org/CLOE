...
class RedshiftBinsTestCase(TestCase):
    ...
    def test_find_bin_scalar(self):
        ...
        npt.assert_equal(
            rb.find_bin(self.edges[0], self.edges), 1, err_msg="Error in find_bin"
        )
        # test zs in 1-st bin
        npt.assert_equal(
            rb.find_bin(self.z_in_1, self.edges), 1, err_msg="Error in find_bin"
        )
        # test zs in 2-nd bin
        npt.assert_equal(
            rb.find_bin(self.z_in_2, self.edges), 2, err_msg="Error in find_bin"
        )
        # test zs below the range without check_bounds
        npt.assert_equal(
            rb.find_bin(self.z_low, self.edges), 0, err_msg="Error in find_bin"
        )
    ...
    def test_find_indices_vector(self):
        ...
        npt.assert_array_equal(
            actual_bins, desired_bins, err_msg="Error in find_bin (list)"
        )
        # test zs in range as a ndarray
        actual_bins = rb.find_bin(test_zs_ndarray, self.edges)
        npt.assert_array_equal(
            actual_bins, desired_bins, err_msg="Error in find_bin (ndarray)"
        )
    ...
    def test_reduce(self):
        ...
        npt.assert_equal(
            rb.reduce(self.edges, 2, 5.0), desired_reduced, err_msg="Error in reduce"
        )
