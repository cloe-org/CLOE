...
np.savetxt(
    opj(savepath, "Cls_kCMB.dat"),
    np.array([ells, cl_kk]).T,
    header="ells     kCMB-kCMB",
)
...
np.savetxt(
    opj(savepath, "Cls_zNLA_ISWxGC.dat"), np.c_[iswxgc_ells, cl_iswxgc.T], header=header
)
...
plt.loglog(
    ells.astype("int"),
    w(ells.astype("int")) * like.cosmo.cosmo_dic["Cl"]["pp"][ells.astype("int")],
    label="CAMB",
)
