...
def setup(options):
    ...
    more_config["zmax_logz"] = options.get_double(opt, "zmax_logz", default=1100.0)
    ...
    if "theta_H0_range" in more_config["cosmology_params"]:
        more_config["cosmology_params"]["theta_H0_range"] = [
    ...
    if (
        cloe_config_dict["likelihood"]["Euclid"]["NL_flag_phot_matter"] == 0
        and cloe_config_dict["likelihood"]["Euclid"]["NL_flag_spectro"] == 0
    ):
        config["NonLinear"] = "NonLinear_none"
        more_config["nonlinear_params"] = {}
    else:
        config["NonLinear"] = "NonLinear_pk"
        likelihood_yaml_handler.set_halofit_version(
            cloe_config_dict,
            cloe_config_dict["likelihood"]["Euclid"]["NL_flag_phot_matter"],
            cloe_config_dict["likelihood"]["Euclid"]["NL_flag_phot_baryon"],
        )
        halofit_version = cloe_config_dict["theory"]["camb"]["extra_args"][
            "halofit_version"
        ]
    ...
    more_config["transfer_params"]["kmax"] = cloe_config_dict["likelihood"]["Euclid"][
        "k_max_extrap"
    ]
    ...
    more_config["kmax_extrapolate"] = cloe_config_dict["likelihood"]["Euclid"][
        "k_max_extrap"
    ]
...
def extract_nonlinear_params(block, config, more_config):
    ...
    if version == "mead2015" or version == "mead2016" or version == "mead2020":
        hmcode_params = {
            "HMCode_A_baryon": block[names.cloe_parameters, "hmcode_a_baryon"],
            "HMCode_eta_baryon": block[names.cloe_parameters, "hmcode_eta_baryon"],
        }
    elif version == "mead2020_feedback":
        hmcode_params = {
            "HMCode_logT_AGN": block[names.cloe_parameters, "hmcode_logt_agn"]
        }
    else:
        hmcode_params = {}
...
def save_distances(r, block, more_config):
    ...
    log_z = np.geomspace(
        more_config["zmax_background"],
        more_config["zmax_logz"],
        num=more_config["n_logz"],
    )
    ...
    D_V = ((1 + z_background) ** 2 * z_background * D_A**2 / H) ** (1.0 / 3.0)
...
def save_matter_power(r, block, more_config):
    ...
    # of these
    kmax_power = max(
        more_config["kmax"] / block[cosmo, "h0"],
        more_config["kmax_extrapolate"] / block[cosmo, "h0"],
    )
    k = np.logspace(
        np.log10(more_config["kmin"] / block[cosmo, "h0"]),
        np.log10(kmax_power),
        more_config["nk"],
    )
    ...
    sigmaR = r.get_sigmaR(np.linspace(5, 50, 100), var1="delta_tot", var2="delta_tot")[
        ::-1
    ]
    sigmaR_cb = r.get_sigmaR(
        np.linspace(5, 50, 100), var1="delta_nonu", var2="delta_nonu"
    )[::-1]
