...
class LikelihoodUI_test(TestCase):
    ...
    def test_init_no_backend_key(self, yaml_read_mock, log_mock):
        ...
        self.assertRaises(KeyError, LikelihoodUI, user_config_file=self.file_name)
    ...
    def test_init_backend_invalid(self, yaml_read_mock, log_mock):
        ...
        self.assertRaises(ValueError, LikelihoodUI, user_config_file=self.file_name)
        self.assertEqual(yaml_read_mock.call_count, 2)
    ...
    @patch(
        "cloe.auxiliary.likelihood_yaml_handler" ".update_cobaya_params_from_model_yaml"
    )
    @patch("cloe.auxiliary.params_converter" ".convert_params")
    @patch(
        "cloe.auxiliary.likelihood_yaml_handler"
    ...
    @patch("cloe.auxiliary.yaml_handler.yaml_read_and_check_dict")
    @patch("cobaya.run")
    def test_run_cobaya_no_cobaya_key(self, cobaya_run_mock, yaml_read_mock, log_mock):
    ...
    def test_run_update_config_not_recursive(self):
        updated = LikelihoodUI._update_config(
            orig_config=self.config_nested, update_config=self.config_nested_update
        )
        self.assertDictEqual(self.config_nested_updated, updated)

    def test_get_and_check_no_backend_key(self):
        self.assertRaises(
            KeyError, LikelihoodUI._get_and_check_backend, self.config_no_backend_key
        )
    ...
    def test_get_and_check_backend_invalid(self):
        self.assertRaises(
            ValueError, LikelihoodUI._get_and_check_backend, self.config_backend_invalid
        )
    ...
    def test_get_and_check_valid_action(
        self, dict_update_mock, yaml_read_mock, log_mock
    ):
        yaml_read_mock.return_value = self.config_good
        dict_update_mock.return_value = self.config_good["Cobaya"]
        ui = LikelihoodUI(user_config_file=self.file_name)
