...
def observables_visualization(observables_dict, palette="standard"):
    ...
    palette: string
        ...
        'deuteranopia'
    """
    observables_df = pd.DataFrame(observables_dict).fillna(-1).astype(int).T
    ...
    cmap = LinearSegmentedColormap.from_list(
        palette, cmaps[palette], len(cmaps[palette])
    )
    ax = sns.heatmap(observables_df, annot=False, cmap=cmap, cbar=True)
...
def observables_selection_checker(observables_dict):
    ...
    observables_dict: dict
        Dictionary with the observables selection
    """
    if observables_dict["WL"]["GCspectro"] or observables_dict["GCphot"]["GCspectro"]:
    ...
    try:
        if observables_dict["CMBlens"]["WL"] and not observables_dict["WL"]["WL"]:
        ...
        if (
            observables_dict["CMBlens"]["GCphot"]
            and not observables_dict["GCphot"]["GCphot"]
        ):
            observables_dict["CMBlens"]["CMBlens"] = True
        ...
        if (
            observables_dict["CMBlens"]["GCspectro"]
            or observables_dict["CMBisw"]["GCspectro"]
        ):
...
def ell_checker(specifications_dict_prob):
    ...
    """
    ells_ranges = np.array(
        [
            specifications_dict_prob["bins"][bin_i][bin_j]["ell_range"][0]
            for bin_i in specifications_dict_prob["bins"]
            for bin_j in specifications_dict_prob["bins"][bin_i]
        ]
    )
...
def observables_selection_specifications_checker(observables_dict, specifications_dict):
    """
    ...
    if checked_observables_dict["GCphot"]["GCphot"]:
        merged_dict["specifications"]["GCphot"] = specifications_dict["GCphot"]
    ...
    if checked_observables_dict["GCspectro"]["GCspectro"]:
        merged_dict["specifications"]["GCspectro"] = specifications_dict["GCspectro"]
    if checked_observables_dict["WL"]["GCphot"]:
        merged_dict["specifications"]["WL-GCphot"] = specifications_dict["WL-GCphot"]
    ...
    if checked_observables_dict["CG"]["CG"]:
        merged_dict["specifications"]["CG"] = specifications_dict["CG"]
    ...
    if checked_observables_dict["GCphot"]["GCspectro"]:
        merged_dict["specifications"]["GCphot-GCspectro"] = specifications_dict[
            "GCphot-GCspectro"
        ]
    if checked_observables_dict["WL"]["GCspectro"]:
        merged_dict["specifications"]["WL-GCspectro"] = specifications_dict[
            "WL-GCspectro"
        ]
    ...
    try:
        if checked_observables_dict["CMBlens"]["CMBlens"]:
            merged_dict["specifications"]["CMBlens"] = specifications_dict["CMBlens"]
        if checked_observables_dict["CMBlens"]["WL"]:
            merged_dict["specifications"]["CMBlens-WL"] = specifications_dict[
                "CMBlens-WL"
            ]
        if checked_observables_dict["CMBlens"]["GCphot"]:
            merged_dict["specifications"]["CMBlens-GCphot"] = specifications_dict[
                "CMBlens-GCphot"
            ]
        if checked_observables_dict["CMBisw"]["GCphot"]:
            merged_dict["specifications"]["ISW-GCphot"] = specifications_dict[
                "ISW-GCphot"
            ]
    except KeyError:
