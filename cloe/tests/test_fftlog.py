...
class fftloginitTestCase(TestCase):
    ...
    def setUp(self) -> None:
        # Check values
        self.a = 1
        self.k = np.logspace(-5.0, 1.0, 2**10)
        self.fk = f(self.k)
        self.rtol = 1e-6
        self.atol = 1e-3
    ...
    def test_hankel(self):
        ...
        real_Fr = F(r)
        npt.assert_allclose(real_Fr, Fr, atol=self.atol, err_msg="FFTLog test failed")
    ...
    def test_fftlog_dj(self):
        ...
        npt.assert_allclose(
            self.check_bl_Fr1, Fr_dj, rtol=self.rtol, err_msg="FFTLog_dj test failed"
        )
    ...
    def test_fftlog_ddj(self):
        ...
        npt.assert_allclose(
            self.check_bl_Fr2, Fr_dj, rtol=self.rtol, err_msg="FFTLog_ddj test failed"
        )
