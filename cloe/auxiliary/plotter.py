...
class Plotter:
    ...
    def __init__(self, cosmo_dic, data, settings=default_settings):
        ...
        self.phot_ins = Photo(
            cosmo_dic, self.read_data.nz_dict_WL, self.read_data.nz_dict_GC_Phot
        )
        ...
        self.nl = settings["nl"]
        self.ells = self._set_binning(self.lmin, self.lmax, self.nl, settings["ltype"])
        ...
        self.nk = settings["nk"]
        self.ks = self._set_binning(self.kmin, self.kmax, self.nk, settings["ktype"])
    ...
    def _set_binning(self, xmin, xmax, nx, btype):
        ...
        if btype == "lin":
            ...
            raise ValueError("Bin type has to be chosen from " "['lin', 'log']")
    ...
    def plot_Cl_phot(
        ...
        pl_ax.plot(
            ells, cl_arr, label=pl_label, color=pl_colour, linestyle=pl_linestyle
        )
        return pl_ax

    def plot_external_Cl_phot(
        self,
        bin_m,
        bin_n,
        pl_ax,
        ...
        pl_ax.plot(
            ells, ext_cs, label=pl_label, color=pl_colour, linestyle=pl_linestyle
        )
        ...
        if probe == "WL":
            ...
            i_low = nbins_wl * nells_wl + nbins_xc * nells_xc + cur_index * nells_gc
            i_high = (
                nbins_wl * nells_wl + nbins_xc * nells_xc + (cur_index + 1) * nells_gc
            )
        err_arr = cov_diags[i_low:i_high]

        pl_ax.plot(ells, ext_cs + err_arr, color=pl_colour, linestyle=pl_linestyle)
        pl_ax.plot(ells, ext_cs - err_arr, color=pl_colour, linestyle=pl_linestyle)
        pl_ax.fill_between(
            ells, ext_cs - err_arr, ext_cs + err_arr, color=pl_colour, alpha=0.2
        )
        return pl_ax
    ...
    def plot_external_Cl_XC(
        ...
        if pl_label is None:
            pl_label = "OU-LE3 WL Bin {:d} - GC-Phot Bin {:d}".format(bin_WL, bin_GC)
        pl_ax.plot(
            ells, ext_cs, label=pl_label, color=pl_colour, linestyle=pl_linestyle
        )
    ...
    def plot_GC_spectro_multipole(
        ...
        pk_arr = np.array(
            [
                self.spec_ins.multipole_spectra(redshift, k_val, ms=[multipole_order])
                for k_val in ks
            ]
        )
        if pl_label is None:
            pl_label = "l={:d}".format(multipole_order)
        pl_ax.plot(ks, pk_arr, label=pl_label, color=pl_colour, linestyle=pl_linestyle)
        return pl_ax

    def plot_external_GC_spectro(
        self,
        redshift,
        multipole_order,
        pl_ax,
        ...
        pl_ax.plot(samp_ks, pk, label=pl_label, color=pl_colour, linestyle=pl_linestyle)
        pl_ax.plot(samp_ks, pk + epk, color=pl_colour, linestyle=pl_linestyle)
        pl_ax.plot(samp_ks, pk - epk, color=pl_colour, linestyle=pl_linestyle)
        pl_ax.fill_between(samp_ks, pk - epk, pk + epk, color=pl_colour, alpha=0.2)
        return pl_ax
    ...
    def output_Cl_WL(self):
        ...
        ax = self.plot_external_Cl_phot(
            self.binX, self.binY, ax, probe="WL", pl_label="Benchmark"
        )
        ax = self.plot_Cl_phot(
            self.ells,
            self.binX,
            self.binY,
            ax,
        ...
        self._set_ax(
            ax,
            title=f"Weak-Lensing bins {self.binX}-{self.binY}",
            xlabel=r"$\ell$",
            ylabel=r"$C_\ell$ $[\mathrm{sr}^{-1}]$",
            fontsize=20,
            xscale="log",
            yscale="log",
        )
        ...
        c_func = self.phot_ins.Cl_WL
        cl_arr = np.array(
            [c_func(cur_ell, self.binX, self.binY) for cur_ell in self.ells]
        )
        np.savetxt(
            f"{filename}.dat",
            list(zip(self.ells, cl_arr)),
            fmt="%.12e",
            delimiter="\t",
            newline="\n",
    ...
    def output_Cl_phot(self):
        ...
        ax = fig.add_subplot(1, 1, 1)
        ax = self.plot_external_Cl_phot(
            self.binX, self.binY, ax, probe="GC-Phot", pl_label="Benchmark"
        )
        ax = self.plot_Cl_phot(
            self.ells,
            self.binX,
        ...
        self._set_ax(
            ax,
            title=f"GC-photo bins {self.binX}-{self.binY}",
            xlabel=r"$\ell$",
            ylabel=r"$C_\ell$ $[\mathrm{sr}^{-1}]$",
            fontsize=20,
            xscale="log",
    ...
    def output_Cl_XC(self):
        ...
        fig = plt.figure()
        ax = fig.add_subplot(1, 1, 1)
        ax = self.plot_external_Cl_XC(self.binX, self.binY, ax, pl_label="Benchmark")
        ax = self.plot_Cl_XC(
            self.ells,
            self.binX,
            self.binY,
        ...
        self._set_ax(
            ax,
            title=f"WL x GC-photo bins {self.binX}-{self.binY}",
            xlabel=r"$\ell$",
            ylabel=r"$C_\ell$ $[\mathrm{sr}^{-1}]$",
            fontsize=20,
            xscale="log",
        ...
        cl_arr = np.array(
            [
                self.phot_ins.Cl_cross(cur_ell, self.binX, self.binY)
    ...
    def output_GC_spectro(self):
        ...
        if self.redshift in [1.0, 1.2, 1.4, 1.65]:
            ax = self.plot_external_GC_spectro(
                str(self.redshift), 0, ax, pl_colour="blue"
            )
            ax = self.plot_external_GC_spectro(
                str(self.redshift), 2, ax, pl_colour="royalblue"
            )
            ax = self.plot_external_GC_spectro(
                str(self.redshift), 4, ax, pl_colour="dodgerblue"
            )
        ax = self.plot_GC_spectro_multipole(
            self.redshift,
            self.ks,
            0,
            ax,
        ...
        self._set_ax(
            ax,
            title=f"GC-spectro $z={self.redshift}$",
        ...
        ax.legend([l1, l2, l3], ["$l=0$", "$l=2$", "$l=4$"])
        plt.tight_layout()
        ...
        pk0 = np.array(
            [
                self.spec_ins.multipole_spectra(self.redshift, k_val, [0])
                for k_val in self.ks
            ]
        )
        pk2 = np.array(
            [
                self.spec_ins.multipole_spectra(self.redshift, k_val, [2])
                for k_val in self.ks
            ]
        )
        pk4 = np.array(
            [
                self.spec_ins.multipole_spectra(self.redshift, k_val, [4])
                for k_val in self.ks
            ]
        )
    ...
    def _set_ax(
        self, axes, title, xlabel, ylabel, fontsize=20, xscale="linear", yscale="linear"
    ):
