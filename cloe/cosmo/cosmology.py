...
class Cosmology:
    ...
    def __init__(self):
        ...
        self.cosmo_dic = {  # Constants
            ...
            "Pk_cb_halomodel_nuapproximation": None,
            "Pk_delta": None,
            "Pk_cb": None,
            ...
            "nuisance_parameters": {
                ...
                "dz_9_GCphot": -0.00929,
                "dz_9_WL": -0.00929,
                "dz_10_GCphot": -0.02095,
                "dz_10_WL": -0.02095,
                "dz_11_GCphot": -0.00782,
                "dz_11_WL": -0.00782,
                "dz_12_GCphot": -0.00108,
                "dz_12_WL": -0.00108,
                "dz_13_GCphot": -0.04686,
                "dz_13_WL": -0.04686,
            },
        }
        ...
        self.nonlinear_neutrino_approx = NonlinearNeutrinoApprox(self.cosmo_dic)
    ...
    def growth_rate(self, zs, ks):
        ...
        try:
            ...
            return interpolate.InterpolatedUnivariateSpline(x=zs, y=f_z_k, ext=2)
        except CosmologyError:
    ...
    def interp_growth_rate(self):
        ...
        if self.cosmo_dic["use_gamma_MG"]:
            ...
            growth = fs8 / s8
        self.cosmo_dic["f_z"] = interpolate.InterpolatedUnivariateSpline(
            x=z_win, y=growth, ext=2
        )
    ...
    def growth_factor_MG(self):
        ...
        """
        integral = [
            quad(self._growth_integrand_MG, z, self.cosmo_dic["z_win"][-1])[0]
            for z in self.cosmo_dic["z_win"]
        ]
    ...
    def assign_growth_factor(self):
        ...
        if self.cosmo_dic["use_gamma_MG"]:
            z_win = self.cosmo_dic["z_win"]
            self.cosmo_dic["D_z_k_func_MG"] = interpolate.interp1d(
                z_win, self.growth_factor_MG(), kind="cubic"
            )
    ...
    def interp_comoving_dist(self):
        ...
        self.cosmo_dic["r_z_func"] = interpolate.InterpolatedUnivariateSpline(
            x=self.cosmo_dic["z_win_max"], y=self.cosmo_dic["comov_dist"], ext=2
        )
    ...
    def interp_z_of_r(self):
        ...
        self.cosmo_dic["z_r_func"] = interpolate.InterpolatedUnivariateSpline(
            x=self.cosmo_dic["comov_dist"], y=self.cosmo_dic["z_win_max"], ext="zeros"
        )
    ...
    def interp_transverse_comoving_dist(self):
        ...
        self.cosmo_dic["f_K_z_func"] = interpolate.InterpolatedUnivariateSpline(
            x=self.cosmo_dic["z_win"], y=transverse_comoving_dist, ext=2
        )
    ...
    def interp_transverse_comoving_dist_z12(self):
        ...
        int_z1z2 = (
            (comov_dist[None, :] - comov_dist[:, None])
            * self.cosmo_dic["H0"]
            / self.cosmo_dic["c"]
        )
        if self.cosmo_dic["Omk"] == 0.0:
            ...
            y_int = np.sinh(np.sqrt(self.cosmo_dic["Omk"]) * int_z1z2) / np.sqrt(
                self.cosmo_dic["Omk"]
            )
        else:
            y_int = np.sin(np.sqrt(-self.cosmo_dic["Omk"]) * int_z1z2) / np.sqrt(
                -self.cosmo_dic["Omk"]
            )
        y_int *= self.cosmo_dic["c"] / self.cosmo_dic["H0"]

        self.cosmo_dic["_f_K_z12_func"] = interpolate.RectBivariateSpline(
            x_int, x_int, y_int, kx=3, ky=3
        )
    ...
    def interp_H_Mpc(self):
        ...
        """
        self.cosmo_dic["H_z_func_Mpc"] = interpolate.InterpolatedUnivariateSpline(
    ...
    def interp_sigma8(self):
        ...
        """
        self.cosmo_dic["sigma8_z_func"] = interpolate.InterpolatedUnivariateSpline(
            x=self.cosmo_dic["z_win"], y=self.cosmo_dic["sigma8"], ext=2
        )

    def interp_cb_growth(self):
        ...
        Pk_grid = self.cosmo_dic["Pk_cb"].P(z, kh)
        Pk_at0_grid = self.cosmo_dic["Pk_cb"].P(0, kh)
        D_cb_grid = np.sqrt(Pk_grid / Pk_at0_grid[None, :])

        self.cosmo_dic["D_cb_z_k_func"] = interpolate.RectBivariateSpline(
            z, kh, D_cb_grid
        )
        ...
        f_cb_grid = (
            -1
            * (1 + z[:, None])
            * self.cosmo_dic["D_cb_z_k_func"](z, kh, 1, 0)
            / D_cb_grid
        )
        self.cosmo_dic["f_cb_z_k_func"] = interpolate.RectBivariateSpline(
            z, kh, f_cb_grid
        )

        fs8_cb_grid = f_cb_grid * self.cosmo_dic["sigma8_cb_z_func"](z)[:, None]
        self.cosmo_dic["fsigma8_cb_z_k_func"] = interpolate.RectBivariateSpline(
            z, kh, fs8_cb_grid
        )
    ...
    def interp_fsigma8(self):
        ...
        """
        self.cosmo_dic["fsigma8_z_func"] = interpolate.InterpolatedUnivariateSpline(
    ...
    def interp_sigmaR(self):
        ...
        if self.cosmo_dic["r_win"] is None:
            raise Exception(
                "Boltzmann code radius binning has not been " "supplied to cosmo_dic."
            )
        self.cosmo_dic["sigmaR_z_func"] = interpolate.RectBivariateSpline(
    ...
    def interp_sigmaR_cb(self):
        ...
        self.cosmo_dic["sigmaR_z_func_cb"] = interpolate.RectBivariateSpline(
            self.cosmo_dic["z_win"],
            self.cosmo_dic["r_win"],
        ...
        self.cosmo_dic["sigma8_cb_z_func"] = partial(
            self.cosmo_dic["sigmaR_z_func_cb"].__call__,
            y=R,
            grid=False,
        )

    def create_phot_galbias(self, model=None, x_values=[0.0, 4.0], y_values=[1.0, 1.0]):
        ...
        if bias_model == 1:
            self.cosmo_dic["b1_inter"] = self.istf_phot_galbias_interpolator(
    ...
    def istf_spectro_galbias(self, redshift):
        ...
        try:
            ...
            bi_val = np.array(
                [nuisance_src[f"b1_spectro_bin{i}"] for i in np.nditer(z_bin)]
            )
    ...
    def Pgg_phot_def(self, redshift, k_scale):
        ...
        """
        pval = (self.compute_phot_galbias(redshift) ** 2) * self.cosmo_dic[
            "Pk_delta"
        ].P(redshift, k_scale)
        return pval
    ...
    def Pgdelta_phot_def(self, redshift, k_scale):
        ...
        Photometric galaxy-matter power spectrum: float or numpy.ndarray
            ...
            photometric
        """
        pval = self.compute_phot_galbias(redshift) * self.cosmo_dic["Pk_delta"].P(
            redshift, k_scale
        )
        return pval
    ...
    def fia(self, redshift, wavenumber=0.001):
        ...
        if self.cosmo_dic["use_gamma_MG"]:
            ...
            growth = self.cosmo_dic["D_z_k_func"](redshift, wavenumber)
        ...
        c1 = 0.0134
        pivot_redshift = self.cosmo_dic["nuisance_parameters"]["pivot_redshift"]
    ...
    def Pii_def(self, redshift, k_scale):
        ...
        """
        pval = self.fia(redshift) ** 2.0 * self.cosmo_dic["Pk_delta"].P(
            redshift, k_scale
        )
        return pval
    ...
    def Pdeltai_def(self, redshift, k_scale):
        ...
        Matter-intrinsic power spectrum: float or numpy.ndarray
            ...
            at a given redshift and wavenumber
        """
        pval = self.fia(redshift) * self.cosmo_dic["Pk_delta"].P(redshift, k_scale)
        return pval
    ...
    def Pgi_phot_def(self, redshift, k_scale):
        ...
        pval = (
            self.fia(redshift)
            * self.compute_phot_galbias(redshift)
    ...
    def Pgi_spectro_def(self, redshift, k_scale):
        ...
        pval = (
            self.fia(redshift)
            * self.istf_spectro_galbias(redshift)
            * self.cosmo_dic["Pk_delta"].P(redshift, k_scale)
        )
        return pval
    ...
    def obtain_power_spectra(self):
        ...
        pmm_phot = np.array([pksrc_phot.Pmm_phot_def(zz, k_win) for zz in z_win])
        pgg_phot = np.array([pksrc_phot.Pgg_phot_def(zz, k_win) for zz in z_win])
        pgdelta_phot = np.array(
            [pksrc_phot.Pgdelta_phot_def(zz, k_win) for zz in z_win]
        )
        pii = np.array([pksrc_phot.Pii_def(zz, k_win) for zz in z_win])
        pdeltai = np.array([pksrc_phot.Pdeltai_def(zz, k_win) for zz in z_win])
        pgi_phot = np.array([pksrc_phot.Pgi_phot_def(zz, k_win) for zz in z_win])
        pgi_spectro = np.array(
            [pksrc_phot.Pgi_spectro_def(zz, k_win) for zz in z_win_spectro]
        )

        self.cosmo_dic["Pgg_spectro"] = pksrc_spectro.Pgg_spectro_def
        self.cosmo_dic["Pgdelta_spectro"] = pksrc_spectro.Pgdelta_spectro_def

        self.cosmo_dic["Pmm_phot"] = interpolate.RectBivariateSpline(
            z_win, k_win, pmm_phot, kx=1, ky=1
        )
        self.cosmo_dic["Pgg_phot"] = interpolate.RectBivariateSpline(
            z_win, k_win, pgg_phot, kx=1, ky=1
        )
        self.cosmo_dic["Pgdelta_phot"] = interpolate.RectBivariateSpline(
            z_win, k_win, pgdelta_phot, kx=1, ky=1
        )
        self.cosmo_dic["Pii"] = interpolate.RectBivariateSpline(
            z_win, k_win, pii, kx=1, ky=1
        )
        self.cosmo_dic["Pdeltai"] = interpolate.RectBivariateSpline(
            z_win, k_win, pdeltai, kx=1, ky=1
        )
        self.cosmo_dic["Pgi_phot"] = interpolate.RectBivariateSpline(
            z_win, k_win, pgi_phot, kx=1, ky=1
        )
        self.cosmo_dic["Pgi_spectro"] = interpolate.RectBivariateSpline(
            z_win_spectro, k_win, pgi_spectro, kx=1, ky=1
        )
        return
    ...
    def rescaled_linear_power_MG(self, redshift, wavenumber):
        ...
        """
        ratio = self.cosmo_dic["D_z_k_func_MG"](redshift) / self.cosmo_dic[
            "D_z_k_func"
        ](redshift, 0.05)
        ...
        return self.cosmo_dic["Pk_delta_Boltzmann"].P(redshift, wavenumber) * ratio**2
    ...
    def rescaled_linear_power_cb_MG(self, redshift, wavenumber):
        ...
        return self.cosmo_dic["Pk_cb_Boltzmann"].P(redshift, wavenumber) * ratio**2

    def rescaled_linear_power_nunonu_MG(self, redshift, wavenumber):
        r"""Rescaled linear neutrino x cb power spectrum due to MG growth
        ...
        return self.cosmo_dic["Pk_nunonu_Boltzmann"].P(redshift, wavenumber) * ratio**2

    def rescaled_linear_power_nunu_MG(self, redshift, wavenumber):
        r"""Rescaled linear neutrino auto power spectrum due to MG growth
        ...
        return self.cosmo_dic["Pk_nunu_Boltzmann"].P(redshift, wavenumber) * ratio**2
    ...
    def rescaled_halomodel_power_MG(self, redshift, wavenumber):
        ...
        return (
            self.cosmo_dic["Pk_halomodel_recipe_Boltzmann"].P(redshift, wavenumber)
            * ratio**2
        )
    ...
    def update_cosmo_dic(self, zs, ks, MG_mu=1.0, MG_sigma=1.0):
        ...
        if self.cosmo_dic["z_win"] is None:
            ...
            raise Exception(
                "Extended redshift binning has not been " "supplied to cosmo_dic."
            )
        self.interp_H()
        self.interp_H_Mpc()
        ...
        self.cosmo_dic["sigma8_0"] = self.cosmo_dic["sigma8_z_func"](0)
        self.cosmo_dic["MG_mu"] = lambda x, y: self.MG_mu_def(x, y, MG_mu)
        self.cosmo_dic["MG_sigma"] = lambda x, y: self.MG_sigma_def(x, y, MG_sigma)
        ...
        self.cosmo_dic["Pk_delta"] = deepcopy(self.cosmo_dic["Pk_delta_Boltzmann"])
        self.cosmo_dic["Pk_cb"] = deepcopy(self.cosmo_dic["Pk_cb_Boltzmann"])
        if self.cosmo_dic["NL_flag_phot_matter"] > 0:
            self.cosmo_dic["Pk_halomodel_recipe"] = deepcopy(
                self.cosmo_dic["Pk_halomodel_recipe_Boltzmann"]
            )
        if self.cosmo_dic["use_gamma_MG"]:
            self.cosmo_dic["Pk_delta"].P = self.rescaled_linear_power_MG
            self.cosmo_dic["Pk_cb"].P = self.rescaled_linear_power_cb_MG
            self.cosmo_dic["Pk_nunonu_Boltzmann"].P = (
                self.rescaled_linear_power_nunonu_MG
            )
            self.cosmo_dic["Pk_nunu_Boltzmann"].P = self.rescaled_linear_power_nunu_MG
            if self.cosmo_dic["NL_flag_phot_matter"] > 0:
                self.cosmo_dic["Pk_halomodel_recipe"].P = (
                    self.rescaled_halomodel_power_MG
                )
                # nonlinear cb power spectrum is automatrically rescaled

        self.interp_cb_growth()

        if self.cosmo_dic["NL_flag_phot_matter"] > 0:
            self.nonlinear_neutrino_approx.wrap_linear_neutrino_approx(
                self.cosmo_dic["Pk_halomodel_recipe"]
            )
            self.cosmo_dic["Pk_cb_halomodel_nuapproximation"] = SimpleNamespace(
                P=self.nonlinear_neutrino_approx
            )
        ...
        if self.cosmo_dic["use_Weyl"]:
            self.cosmo_dic["Weyl_matter_ratio"] = self.Weyl_matter_ratio_def

        self.cosmo_dic["noise_Pgg_spectro"] = self.pk_source_spectro.noise_Pgg_spectro
