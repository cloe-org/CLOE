...
class CG:
    ...
    def __init__(self, cosmo_dic):
        ...
        self.profile = self.theory["observables_specifications"]["CG"]["halo_profile"]
        self.overdensity_type = self.theory["observables_specifications"]["CG"][
            "overdensity_type"
        ]
        self.overdensity = self.theory["observables_specifications"]["CG"][
            "overdensity"
        ]
        ...
        self.area = self.theory["observables_specifications"]["CG"]["effective_area"]

        # Observed redshift and richness binning
        self.z_obs_edges_CC = self.theory["observables_specifications"]["CG"][
            "z_obs_edges_CC"
        ]
        self.lambda_obs_edges_CC = self.theory["observables_specifications"]["CG"][
            "Lambda_obs_edges_CC"
        ]
        ...
        ]["CG"]["external_richness_selection_function"]
        self.selection_function = self.theory["observables_specifications"]["CG"][
        ...
        if self.external_richness_selection_function == "CG_ESF":
            ...
            lambda_grid = 10.0 ** (np.linspace(np.log10(5.0), np.log10(100.0), 100))
        ...
        self.sig_A_l = self.theory["observables_specifications"]["CG"]["sig_A_l"]
        self.sig_B_l = self.theory["observables_specifications"]["CG"]["sig_B_l"]
        self.sig_C_l = self.theory["observables_specifications"]["CG"]["sig_C_l"]

        self.M_pivot = float(self.theory["observables_specifications"]["CG"]["M_pivot"])
        self.z_pivot = self.theory["observables_specifications"]["CG"]["z_pivot"]

        # Parameters defining the scatter of the observed-true richness PDF
        self.sig_lambda_norm = self.theory["observables_specifications"]["CG"][
            "sig_lambda_norm"
        ]
        self.sig_lambda_z = self.theory["observables_specifications"]["CG"][
            "sig_lambda_z"
        ]
        self.sig_lambda_exponent = self.theory["observables_specifications"]["CG"][
            "sig_lambda_exponent"
        ]

        # Parameters defining the scatter of the observed-true redshift PDF
        self.sig_z_norm = self.theory["observables_specifications"]["CG"]["sig_z_norm"]
        self.sig_z_z = self.theory["observables_specifications"]["CG"]["sig_z_z"]
        self.sig_z_lambda = self.theory["observables_specifications"]["CG"][
            "sig_z_lambda"
        ]
        ...
        lambda_max = 250.0
        lambda_div = 50
        self.Lambda = np.linspace(lambda_min, lambda_max, lambda_div + 1)
    ...
    def rho_crit_0(self):
        ...
        hubble_value = self.theory["H0"] / 3.085677581491367e19
    ...
    def sigma_z_M(self, z, M):
        ...
        if self.neutrino_cdm == "cb":
            return self.theory["sigmaR_z_func_cb"](z, self.radius_M(M), grid=True)
        else:
    ...
    def dndm(self, z, M):
        ...
        dndm = self.rho_mean_0() / M**2.0 * self.f_sigma_nu(z, M) * dlnsigmadlnR / (-3)
        return dndm
    ...
    def lnlambda(self, z, M):
        ...
        lnlambda = (
            np.log(self.A_l)
            + self.B_l * np.log(M / self.M_pivot)
            + self.C_l * np.log((1.0 + z[:, np.newaxis]) / (1.0 + self.z_pivot))
        )
        return lnlambda
    ...
    def scatter_lnlambda(self, z, M):
        ...
        return (
            self.sig_A_l
            + self.sig_B_l * np.log(M / self.M_pivot)
            + self.sig_C_l * np.log((1.0 + z[:, np.newaxis]) / (1.0 + self.z_pivot))
        )
    ...
    def scatter_z_obs(self, Lambda_obs, z):
        ...
        return self.sig_z_norm + self.sig_z_z * z + self.sig_z_lambda * Lambda_obs
    ...
    def Pz_obs(self, z_obs, Lambda_obs, z):
        ...
        return (
            1.0
            / (np.sqrt(2.0 * np.pi * scatter**2.0))
            * np.exp(-((z_obs[:, np.newaxis] - z) ** 2.0) / (2.0 * scatter**2.0))
        )
    ...
    def halo_bias(self, z, M, Delta):
        ...
        return (
            1.0
            - A_par * b_nu**a_par / (b_nu**a_par + self.delta_c(z) ** a_par)
            + B_par * b_nu**b_par
            + C_par * b_nu**c_par
        ).T
    ...
    def cov_window(self, zbin, ztab, kh, L):
        ...
        self.rint[zbin] = (
            ...
            * simps(
                rvec**2.0
                * np.array(
                    [spherical_jn(ell, kr, derivative=False) for ell in range(L + 1)]
                ),
                rvec,
                axis=-1,
            ).T
        )
        cov_window = (4 * np.pi) * np.sum(
            self.rint[zbin, :, :] * self.rint[: (zbin + 1), :, :] * self.KL[:] ** 2,
            axis=-1,
        )
        return cov_window
    ...
    def N_zbin_Lbin_Rbin(self):
        ...
        CG_like_selection = self.theory["observables_specifications"]["CG"]["CG_probe"]
        CG_xi2_cov_selection = self.theory["observables_specifications"]["CG"][
        ...
        dzobs_dz = np.empty((self.z_obs_div_CC, self.lambda_obs_div_CC), dtype=list)

        if CG_like_selection in ["CC", "CC_CWL", "CC_Cxi2", "CC_CWL_Cxi2"]:

            # Starting of galaxy cluster number counts computation
            z_obs_mid = 0.5 * (self.z_obs_edges_CC[1:] + self.z_obs_edges_CC[:-1])
            ...
            if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                ...
                Nb_zbin_Lbin = np.zeros((self.z_obs_div_CC, self.lambda_obs_div_CC))
            ...
            for lambda_bin in range(len(self.lambda_obs_edges_CC) - 1):
                ...
                Plob_l_z = simps(
                    self.Plambda_obs(z_mid, lambda_mid, l_tab), l_tab, axis=-1
                )

                if external_richness_selection_function == "CG_ESF":
                    Plob_l_z = self.interp_Plambda_obs[lambda_bin](z_mid, lambda_mid).T

                # calculation of the Probability distribution relating
                # richness to mass and redshift
                Pl_M_z = self.Pscaling_relation(z_mid, 10.0**self.Mass, lambda_mid)
                Plob_M_z[lambda_bin] = simps(
                    Pl_M_z[:, :, :] * Plob_l_z[:, np.newaxis, :], lambda_mid, axis=-1
                )
                ...
                n_lbdobs_z[lambda_bin] = simps(
                    Plob_M_z[lambda_bin] * dndm_z, 10.0**self.Mass, axis=1
                )
                ...
                if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                    b_n_lbdobs_z[lambda_bin] = simps(
                        Plob_M_z[lambda_bin] * dndm_z * bias_z, 10.0**self.Mass, axis=1
                    )
                ...
                for z_bin in range(len(self.z_obs_edges_CC) - 1):
                    ...
                    dzobs_dz[z_bin, lambda_bin] = simps(
                        self.Pz_obs(z_tab, self.lambda_obs_edges_CC[lambda_bin], z_mid),
                        z_tab,
                        axis=0,
                    )
            ...
            if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                ...
                cov_zbin_Lbin = SN + (
                    ...
                    * Nb_zbin_Lbin.reshape(
                        self.z_obs_div_CC, 1, self.lambda_obs_div_CC, 1
                    )
                    * sab.reshape(self.z_obs_div_CC, self.z_obs_div_CC, 1, 1)
                )

        g_zbin_Lbin_Rbin = np.zeros(((self.z_obs_div_CC, self.lambda_obs_div_CC, 1)))
