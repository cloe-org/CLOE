...
class nonlinearinitTestCase(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cosmo_dic_extra = load_test_pickle("cosmo_test_NLphot3_extra_dic.pickle")
        ...
        nonlinear_dic = load_test_pickle("cosmo_test_NLspectro1_nonlin_dic.pickle")
        ...
        # Load cosmology dictionary for cosmology extrapolation tests
        cosmo_dic_bar3_extra = load_test_pickle("cosmo_test_NLphot3_extra_dic.pickle")
        ...
        cosmo_dic_bar3_extra["nuisance_parameters"]["log10Mc_bcemu_bin1"] = 15.5
        # Create instance of Nonlinear class and update its dictionary
        ...
        # Load cosmology dictionary for cosmology extrapolation tests
        cosmo_dic_bar4_extra = load_test_pickle("cosmo_test_NLphot3_extra_dic.pickle")
        ...
        # Load cosmology dictionary for tests with NL_flag_phot_matter=1
        # and IA_flag = 1
        cosmo_dic_1_tatt = load_test_pickle("cosmo_test_NLphot1_tatt_dic.pickle")
        # Create instance of Nonlinear class
        cls.nl1_tatt = Nonlinear(cosmo_dic_1_tatt)

        # Load cosmology dictionary for tests with NL_flag_phot_matter=2
        # and IA_flag = 1
        cosmo_dic_2_tatt = load_test_pickle("cosmo_test_NLphot2_tatt_dic.pickle")
        # Create instance of Nonlinear class
        cls.nl2_tatt = Nonlinear(cosmo_dic_2_tatt)

        # Load cosmology dictionary for tests with NL_flag_phot_matter=3
        # and IA_flag = 1
        cosmo_dic_3_tatt = load_test_pickle("cosmo_test_NLphot3_tatt_dic.pickle")
        # Create instance of Nonlinear class
    ...
    def setUp(self) -> None:
        ...
        self.Pii_test_NL1_tatt = np.array([7.11749416, 6.52936089, 5.24846405])
        self.Pdeltai_test_NL1_tatt = -277.24173292
        self.Pgi_phot_test_NL1_tatt = -392.01731165

        self.Pii_test_NL2_tatt = np.array([7.12113817, 6.52871957, 5.24909852])
        self.Pdeltai_test_NL2_tatt = -277.64232328
        self.Pgi_phot_test_NL2_tatt = -392.58369074

        self.Pii_test_NL3_tatt = np.array([7.12611501, 6.5281627, 5.24907609])
        self.Pdeltai_test_NL3_tatt = -278.18941671
        self.Pgi_phot_test_NL3_tatt = -393.35722993

        self.Pii_test_NL4_tatt = np.array([7.11287427, 6.52525458, 5.24379997])
        self.Pdeltai_test_NL4_tatt = -276.7730395
        self.Pgi_phot_test_NL4_tatt = -391.37356994

        self.Pii_test_NL5_tatt = np.array([7.12395548, 6.52429, 5.24201532])
    ...
    def test_Pgg_phot_def(self):
        ...
        npt.assert_allclose(
            self.nl1_nlbias_halo.Pgg_phot_def(self.redshift1, self.wavenumber1),
        ...
        npt.assert_allclose(
            self.nl1_nlbias_emu.Pgg_phot_def(self.redshift1, self.wavenumber1),
    ...
    def test_Pgdelta_phot_def(self):
        ...
        npt.assert_allclose(
            self.nl1_nlbias_halo.Pgdelta_phot_def(self.redshift1, self.wavenumber1),
        ...
        npt.assert_allclose(
            self.nl1_nlbias_emu.Pgdelta_phot_def(self.redshift1, self.wavenumber1),
    ...
    def test_Pgg_spectro_def(self):
        npt.assert_allclose(
            self.nl7.Pgg_spectro_def(self.redshift1, self.wavenumber1, self.mu),
    ...
    def test_noise_Pgg_spectro(self):
        npt.assert_allclose(
            self.nl8.noise_Pgg_spectro(self.redshift1, self.wavenumber1, self.mu),
            self.noise_Pgg_spectro,
            rtol=self.rtol,
    ...
    def test_Pgdelta_spectro_def(self):
        npt.assert_allclose(
            self.nl7.Pgdelta_spectro_def(self.redshift1, self.wavenumber1, self.mu),
    ...
    def test_Pii_def(self):
        test_p1 = self.nl1.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )

        type_check = isinstance(test_p1, np.ndarray)

        assert type_check, (
            "Error in returned data type of Pii_def " "for NL_flag_phot_matter=1"
        )

        assert test_p1.size == self.arrsize, (
            "Error in size of array returned by Pii_def for " "NL_flag_phot_matter=1"
        )
        ...
        npt.assert_allclose(
            ...
            self.Pii_test_NL1,
            rtol=self.rtol,
            err_msg="Error in values returned by Pii_def for " "NL_flag_phot_matter=1",
        )

        test_p2 = self.nl2.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        assert type_check, (
            "Error in returned data type of Pii_def " "for NL_flag_phot_matter=2"
        )

        assert test_p2.size == self.arrsize, (
            "Error in size of array returned by Pii_def for " "NL_flag_phot_matter=2"
        )
        ...
        npt.assert_allclose(
            ...
            self.Pii_test_NL2,
            rtol=self.rtol,
            err_msg="Error in values returned by Pii_def for " "NL_flag_phot_matter=2",
        )

        test_p3 = self.nl3.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )

        type_check = isinstance(test_p3, np.ndarray)
        assert type_check, (
            "Error in returned data type of Pii_def " "for NL_flag_phot_matter=3"
        )

        assert test_p3.size == self.arrsize, (
            "Error in size of array " "returned by Pii_def " "for NL_flag_phot_matter=3"
        )
        ...
        npt.assert_allclose(
            ...
            self.Pii_test_NL3,
            rtol=self.rtol,
            err_msg="Error in values returned by Pii_def for " "NL_flag_phot_matter=3",
        )

        test_p4 = self.nl4.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        assert type_check, (
            "Error in returned data type of Pii_def " "for NL_flag_phot_matter=4"
        )

        assert test_p4.size == self.arrsize, (
            "Error in size of array returned by Pii_def for " "NL_flag_phot_matter=4"
        )
        ...
        npt.assert_allclose(
            ...
            self.Pii_test_NL4,
            rtol=self.rtol,
            err_msg="Error in values returned by Pii_def for " "NL_flag_phot_matter=4",
        )

        test_p5 = self.nl5.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )

        type_check = isinstance(test_p5, np.ndarray)
        assert type_check, (
            "Error in returned data type of Pii_def " "for NL_flag_phot_matter=5"
        )

        assert test_p5.size == self.arrsize, (
            "Error in size of array " "returned by Pii_def " "for NL_flag_phot_matter=5"
        )
        ...
        npt.assert_allclose(
            ...
            self.Pii_test_NL5,
            rtol=self.rtol,
            err_msg="Error in values returned by Pii_def for " "NL_flag_phot_matter=5",
        )

        test_p1_tatt = self.nl1_tatt.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        assert type_check, (
            "Error in returned data type of Pii_def "
        ...
        test_p2_tatt = self.nl2_tatt.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        test_p3_tatt = self.nl3_tatt.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        assert test_p3_tatt.size == self.arrsize, (
            "Error in size of array "
            "returned by Pii_def "
            "for NL_flag_phot_matter=3"
            "and IA_flag=1"
        )
        ...
        test_p4_tatt = self.nl4_tatt.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        test_p5_tatt = self.nl5_tatt.Pii_def(
            self.redshift1, [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
        ...
        assert test_p5_tatt.size == self.arrsize, (
            "Error in size of array "
            "returned by Pii_def "
            "for NL_flag_phot_matter=5"
            "and IA_flag=1"
    ...
    def test_fia_k_array(self):
        wavenumber_array = np.array(
            [self.wavenumber1, self.wavenumber2, self.wavenumber3]
        )
    ...
    def test_poly_phot_galbias(self):
        for b in ["1", "2", "G2", "G3"]:
            b_val = self.nl1_nlbias_halo.poly_phot_galbias_nl(f"b{b}")(1.0)
            z_arr = np.array([1.0, 2.0])
            b_val_arr = self.nl1_nlbias_halo.poly_phot_galbias_nl(f"b{b}")(z_arr)
    ...
    def test_baryons_Pmm_phot_def(self):
        ...
        npt.assert_allclose(
            ...
            rtol=self.rtol,
            err_msg="Error in value returned by " + "Pmm_phot_def for BACCO baryons",
        )
        ...
        npt.assert_allclose(
            ...
            rtol=self.rtol,
            err_msg="Error in value returned by " + "Pmm_phot_def for BCemu baryons",
        )
    ...
    def test_baryon_extrapolation_Pmm_phot_def(self):
        ...
        npt.assert_allclose(
            self.nl3_b3.Pmm_phot_def(self.redshift_extra, self.wavenumber_extra),
