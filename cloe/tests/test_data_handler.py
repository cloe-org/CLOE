...
class datahandlerTestCase(TestCase):
    ...
    def setUpClass(self):
        ...
        self.datavec_shape = (
            self.wl_size + self.xc_phot_size + self.gc_phot_size + self.gc_spectro_size,
        )
        ...
        self.xc_phot_cov = np.random.randn(self.xc_phot_size, self.xc_phot_size)
        self.gc_phot_cov = np.random.randn(self.gc_phot_size, self.gc_phot_size)
        self.tx2_cov = np.random.randn(self.tx2_size, self.tx2_size)
        self.gc_spectro_cov = np.random.randn(
            self.gc_spectro_size, self.gc_spectro_size
        )
        ...
        cov = {"3x2pt": self.tx2_cov, "GC-Spectro": self.gc_spectro_cov}

        self.data_handler = Data_handler(data, cov, self.observables, self.data_reader)
        ...
        if not self.observables["selection"]["WL"]["GCphot"]:
            self.masking_vector[self.wl_size : (self.wl_size + self.xc_phot_size)] = 0
        if not self.observables["selection"]["GCphot"]["GCphot"]:
            self.masking_vector[
                (self.wl_size + self.xc_phot_size) : (
                    self.wl_size + self.xc_phot_size + self.gc_phot_size
                )
            ] = 0
    ...
    def test_create_data_vector(self):
        ...
        npt.assert_equal(
            self.data_handler._data_vector.shape,
            self.datavec_shape,
            err_msg=f"Shape of full data vector does not match" f" the expected shape.",
        )
    ...
    def test_create_cov_matrix(self):
        ...
        npt.assert_equal(
            self.data_handler._cov_matrix.shape,
            self.cov_shape,
    ...
    def test_create_masking_vectors(self):
        ...
        npt.assert_equal(
            self.data_handler._masking_vector,
            self.masking_vector,
            err_msg=f"Values of masking vector do not match" f" expected values.",
        )
    ...
    def test_observables_selection(self):
        ...
        use_wl, use_xc_phot, use_gc_phot, use_gc_spectro = np.random.choice(
            [True, False], size=4
        )
        obs = {
            "WL": {"WL": use_wl, "GCphot": use_xc_phot, "GCspectro": False},
    ...
    def test_spectro_data_size(self):
        npt.assert_equal(
            self.data_handler.gc_spectro_size,
            self.gc_spectro_size,
