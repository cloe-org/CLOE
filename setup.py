...
release_info = {}
infopath = os.path.abspath(os.path.join(os.path.dirname(__file__), __name__, "info.py"))
...
def find_scripts():
    sdir = "./scripts"
    return [
        os.path.join(sdir, val)
        for val in os.listdir(sdir)
        if val.endswith(".py") and "__init__" not in val
    ]
