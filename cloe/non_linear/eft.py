...
class EFTofLSS:
    ...
    def CallEH_NW(self, redshift):
        ...
        Tcmb = 2.726
        kL = self.ks
        s = 44.5 * np.log(9.83 / self.Omh2) / np.sqrt(1.0 + 10.0 * (self.Obh2) ** 0.75)
        Gamma = self.Omh2 / self.h
        AG = (
            1.0
            - 0.328 * np.log(431.0 * self.Omh2) * self.Obh2 / self.Omh2
    ...
    def _gaussianFiltering(self, lamb, redshift):
        def extrapolateX(x):
            ...
            newlogx = np.linspace(
                logx[0] - xSize * dlogx / 2, logx[-1] + xSize * dlogx / 2, 2 * xSize
            )
            newx = 10**newlogx
            return newx
        ...
        pLin = extrapolateFx(self.ks, self.linear_power_func.P(redshift, self.ks))
    ...
    def pksmooth_dst(self, redshift):
        ...
        imin_start = (
            np.argmax(even_2nd_der[range_imin[0] : range_imin[1]]) + range_imin[0] - 30
        )
        imax_start = (
            np.where(even_2nd_der[range_imax[0] : range_imax[1]] < 0.04)[0][0]
            + range_imax[0]
        )

        # Cut out the BAOs
        # mask indices
        i_even_holed = np.concatenate((i_even[:imin_start], i_even[imax_start:]))
    ...
    def IRresum(self, IRres="DST", redshift=0.0, lamb=0.25, kS=0.2, lOsc=102.707):
        ...
        Sigma2 = norm * simps(
            ...
            * (
                1.0
                - spherical_jn(0, kLcut / kosc)
                + 2.0 * spherical_jn(2, kLcut / kosc)
            ),
            x=kLcut,
        )
        dSigma2 = norm * simps(3.0 * Pnwcut * spherical_jn(2, kLcut / kosc), x=kLcut)
    ...
    def _muxdamp(self, mu, Sigma2, dSigma2, f):
        ...
        """
        Sig2mu = Sigma2 + f * mu**2 * (
            2.0 * Sigma2 + f * (Sigma2 + dSigma2 * (mu**2 - 1.0))
        )
    ...
    def _Pgg_kmu_terms(self, redshift, IRres="DST"):
        ...
        self.Pnw, self.Pw, self.Sigma2, self.dSigma2 = self.IRresum(
            IRres=IRres, redshift=redshift
        )

        # Loops on P_L and Pnw
        loop22_L = self.fastpt.Pkmu_22_one_loop_terms(self.ks, self.PL, C_window=0.75)
        loop22_nw = self.fastpt.Pkmu_22_one_loop_terms(self.ks, self.Pnw, C_window=0.75)
    ...
    def _Pgg_k_terms_L(self):
        ...
        # Loops on P_L
        loop22_L = self.fastpt.Pk_22_one_loop_terms_realspace(PL, C_window=0.75)
        loop13_L = self.fastpt.Pkmu_13_one_loop_terms(self.ks, PL)
        setattr(self, "loop22_L", loop22_L)
        setattr(self, "loop13_L", loop13_L)

    def P_kmu_z(self, redshift, use_growth_rescaling=False, IRres="DST", **kwargs):
        r"""P_kmu_z.
        ...
        **kwargs: Dictionary containing the model parameters
        ...
        loop22_nw_sub = (
            np.array([self.loop22_nw[i] for i in range(len(self.loop22_nw))]) * D4
        )
        loop13_nw_sub = (
            np.array([self.loop13_nw[i] for i in range(len(self.loop13_nw))]) * D4
        )
        ...
        PNLO = Kaiser(b1, f, mu) ** 2 * (
            Pnw_sub + RSDdamp * Pw_sub * (1.0 + ks_sub**2 * Sig2mu)
        )
        Pkmu_ctr = (
            -2.0 * (c0 + c2 * f * mu**2 + c4 * f * f * mu**4) * ks_sub**2
            + ck4 * f**4 * mu**4 * Kaiser(b1, f, mu) ** 2 * ks_sub**4
        ) * (Pnw_sub + RSDdamp * Pw_sub)
    ...
    def P_realspace_terms_kz(self, D=None):
        ...
        return (
            RectBivariateSpline(self.z, self.ks, Pb1b2),
...
class FASTPTPlus(fpts.FASTPT):
    ...
    def Pkmu_22_one_loop_terms(self, k, P, P_window=None, C_window=None):
        ...
        Pb2bG2 = 4 / 3.0 * (J002 - J000)
        PbG2bG2 = 16 / 15.0 * J000 - 32 / 21.0 * J002 + 16 / 35.0 * J004
