...
class ParamsConverterTestCase(TestCase):
    ...
    def test_camb_to_classy_conversion(self):
        ...
        if not np.all(
            [key in self.params_classy.keys() for key in self.params_camb.keys()]
        ):
            raise KeyError(
                "Unrecognised key while converting CAMB parameters "
                "to CLASS parameters."
            )
    ...
    def test_classy_to_camb_conversion(self):
        ...
        if not np.all(
            [key in self.params_camb.keys() for key in self.params_classy.keys()]
        ):
            raise KeyError(
                "Unrecognised key while converting CLASS "
