...
ells_ISW = np.loadtxt("../data/cmbx/_Cls_cloe_ISWxGC.dat")[:, 0]
...
sigma_T = [33.0]
beam = 7.0
theta2 = (beam * np.array([np.pi / 60.0 / 180.0])) ** 2 / 8.0 / np.log(2.0)
sigma_T *= np.array([np.pi / 60.0 / 180.0])
...
SO_temperature = opj(
    savepath,
    "so_noise_models",
    "LAT_comp_sep_noise/v3.1.0/SO_LAT_Nell_T_atmv1_baseline_fsky0p4_ILC_CMB.txt",
)
...
for i, j in like.likefinal.indices_diagonal_wl:
    ...
    if i == j:
        nl_dict[k] = sig_eps**2 / ngal * np.ones(len(ells)) / 2.0
                ...
                for il2, l2 in enumerate(ells):
                    for z2 in range(len(cl_keys[key2])):
                        if l1 == l2:
                            ...
                            cov_mat_tot[ind1, ind2] = (
                                1.0
                                / (fsky * (2.0 * l + 1.0) * dell[il])
                                * (
                                    (
                                        cl_dict["-".join([probeA, probeC])][il]
                                        + nl_dict["-".join([probeA, probeC])][il]
                                    )
                                    * (
                                        cl_dict["-".join([probeB, probeD])][il]
                                        + nl_dict["-".join([probeB, probeD])][il]
                                    )
                                    + (
                                        cl_dict["-".join([probeA, probeD])][il]
                                        + nl_dict["-".join([probeA, probeD])][il]
                                    )
                                    * (
                                        cl_dict["-".join([probeB, probeC])][il]
                                        + nl_dict["-".join([probeB, probeC])][il]
                                    )
                                )
                            )
                        ind2 += 1
...
for il, l in enumerate(ells_ISW):
    for i in range(numtomo_gcphot):
        for j in range(i, numtomo_gcphot):
            cov_mat_tot[
                len_6x2pt + il * numtomo_gcphot + i, len_6x2pt + il * numtomo_gcphot + j
            ] = (
                1.0
                / (fsky * (2.0 * l + 1.0) * dell_ISW[il])
                * (
                    (
                        cl_GC_ij["GC{}-GC{}".format(i + 1, j + 1)][il]
                        + nl_GC_ij["GC{}-GC{}".format(i + 1, j + 1)][il]
                    )
                    * (cl_TT[il] + nl_TT[il])
